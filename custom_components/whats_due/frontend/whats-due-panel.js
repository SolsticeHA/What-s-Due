...
`;let wt=class extends nt{constructor(){super(...arguments),this.label="",this.value="",this._onChange=t=>{this._emit(t.detail.value)}}render(){return t="ha-icon-picker",void 0!==customElements.get(t)?q`
        <ha-icon-picker
          .label=${this.label}
          .value=${this.value}
  ...
  `,t([pt()],wt.prototype,"label",void 0),t([pt()],wt.prototype,"value",void 0),wt=t([lt("wd-icon-picker")],wt);let At=class extends nt{constructor(){super(...arguments),this.categories=[],this._cancel=()=>{this.dispatchEvent(new CustomEvent("cancel"))},this._save=()=>{this._local.name.trim()&&this._local.due_date&&this.dispatchEvent(new CustomEvent("save",{detail:this._local}))}}willUpdate(t){t.has("draft")&&this.draft&&(this._local={...this.draft})}_patch(t,e){this._local={...this._local,[t]:e},this.requestUpdate()}render(){if(!this._local)return V;const t=this.strings,e=this._local;return q`
      <ha-dialog
        open
